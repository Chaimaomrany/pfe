...
// Helpers
import { accountRequiredValidator } from "@app/accelengine-modules/module-user/validators/account.validator";
...
export class UserMasterDetailComponent
    ...
    valuesAbilities: DictionaryValue[];
    //allAbilitys: Ability[];
    ...
    constructor(
        ...
        this.columnsAbilities = Column.fromObjects([
            { field: 'code', header: "user.label_code" },
            { field: 'label', header: "user.label_label" }
        ]);
    }
    ...
    ngOnInit(): void {
        ...
        this.initUI();
        this.initData();
        this.subscriptions.push(this.dictionaryTypeService.findAllValuesByTypeCode("ABILITY").subscribe((res: DictionaryValue[]) => {
            this.valuesAbilities = res;
        }))
        ...
        this.subscriptions.push(this.settingAppService.findOneByCode(SettingAppService.ACCOUNT).subscribe((result: SettingApp) => {
            ...
            this.formGroup = this.formBuilder.group({
                ...
                {
                    validator: accountRequiredValidator('role', this.settingAppAccount.visibleForRoles)
                });
        }));
        ...
        this.subscriptions.push(this.roleService.getAllActivate().subscribe((result: AEList<Role>) => {
            ...
            let criteriaRole = this.criterias.find((criteria: AECriteria) => criteria.field === "role.id");
            if (criteriaRole) {
