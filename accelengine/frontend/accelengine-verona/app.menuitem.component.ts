    ...
    template: `
          <ng-container *ngIf="visible">
              ...
              <a [attr.href]="item.url" (click)="itemClick($event)" *ngIf="!item.routerLink || item.items" (mouseenter)="onMouseEnter()"
                 ...
                 pTooltip="{{ item.label | translate }}" [tooltipDisabled]="active || !(root && appMain.isSlim() && !appMain.isMobile())" tooltipStyleClass="layout-tooltip" pRipple>
...
export class AppMenuitemComponent implements OnInit, OnDestroy {
    ...
    constructor(public appMain: AppMainComponent, public app: AppComponent, public router: Router,
                private cd: ChangeDetectorRef, private menuService: MenuService, private accountService: AccountService, private storageService: StorageService) {
    ...
    itemClick(event: Event) {
        this.storageService.set(StorageService.CURRENT_MENU_CODE, this.item.code);
        let current_account = this.storageService.getCurrentAccount();
        this.accountService.getAccountSetting(current_account.id, this.item.code).subscribe((res) => {
            if (res) {
                this.storageService.set(this.item.code, res.criterias)
            }
        })
